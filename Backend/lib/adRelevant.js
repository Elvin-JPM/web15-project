  ...
  try {
    ...
    for (const product of products) {
      if(productCreated.owner !== product.owner) {
        try {
          if ( (productCreated.sale === true && product.sale === false) | (productCreated.sale === false && product.sale === true)){
            const userToSendEmail = await User.findOne({ username: product.owner });
            ...
            const productURL = `http://localhost:5173/products/${productCreated.name}/${productCreated._id}`;
