    ...
    async updateUserInfo(req, res, next) {
        try {
            ...
            if (newData.email || newData.username) {
                ...
                const existingUsername = await User.findOne({ username: newData.username });
            ...
            if (newData.username) {
                await Product.updateMany({ owner: username }, { owner: newData.username });
                await Product.updateMany({ "favs": username }, { $set: { "favs.$": newData.username } });
            }

            res.json({ message: 'La información del usuario ha sido actualizada correctamente.', user: updatedUser });
        } catch (err) {
            next(err);
        }
    }
